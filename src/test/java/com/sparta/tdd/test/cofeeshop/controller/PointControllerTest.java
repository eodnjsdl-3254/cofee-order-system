...
import com.sparta.tdd.cofeeshop.model.User; // User 엔티티 임포트 경로 확인
import com.sparta.tdd.cofeeshop.repository.UserRepository; // UserRepository 임포트 경로 확인
...
class PointControllerTest {
    ...
    @Test
    @DisplayName("POST /api/points/charge: 유효한 금액으로 포인트 충전에 성공한다.")
    ...
    @Test
    @DisplayName("POST /api/points/charge: 음수 금액으로 포인트 충전 시 실패하고 INVALID_INPUT 에러를 반환한다.")
    ...
    @Test
    @DisplayName("POST /api/points/charge: 존재하지 않는 사용자 ID로 포인트 충전 시 실패하고 USER_NOT_FOUND 에러를 반환한다.")
